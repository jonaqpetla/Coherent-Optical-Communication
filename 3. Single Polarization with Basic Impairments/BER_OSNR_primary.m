...
osnr_dB_options = 6:2:40;
bit_error_rate = zeros(1, length(osnr_dB_options));
for osnr_iterator = 1:length(osnr_dB_options)
    osnr_linear = 10^(osnr_dB_options(osnr_iterator)*0.1);
    ...
    local_oscillator_power_W = 200e-3;
    local_oscillator_linewidth_Hz = 200e3;
    local_oscillator_center_frequency_Hz = 3e8/1550e-9 + 500e6;
    ...
    scatterplot(received_symbols_CD_FO_PN_corrected);
    bits_in_error = sum(received_bits ~= transmit_bits);
    bit_error_rate(osnr_iterator) = bits_in_error/(no_symbols*bits_per_symbol);
end

figure; semilogy(osnr_dB_options, bit_error_rate, 'b-o', 'linewidth', 1.5);